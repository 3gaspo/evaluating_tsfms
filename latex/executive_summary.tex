...
\author{}
\date{16 September 2026; results computed 15 September 2026}
\begin{document}
\begin{center}
{\Large\bfseries Evaluating Time-Series Foundation Models}\\[4pt]
{\large Scientific Executive Summary}\\[5pt]
{\small 16 September 2026; results computed 15 September 2026}
\end{center}
...
It also has the lowest measured total forecasting time (425.6 seconds).
...
same accuracy as native input, with 5.72 times its measured forecasting time.
...
Their total forecasting times are 6.28 and 2.20 times Chronos-2's.
\end{itemize}
...
\midrule
Native multivariate & 0.690045 & reference & 326.8 & 1.00\\
Independent univariate & 0.695590 & $+0.80\%$ & 341.8 & 1.05\\
Past-target covariates & 0.690045 & equivalent & 1,870.6 & 5.72\\
\bottomrule
\end{tabularx}
...
relative difference is below $5.75\times10^{-7}$. They take \textbf{5.72
...
\begin{figure}[h]
\centering
\includegraphics[width=0.88\linewidth]{executive_summary_feature_scatter.png}
...
and spikiness.
These comparisons concern different existing datasets: changes in
heterogeneity occur alongside other dataset differences, so the association
does not isolate the effect of heterogeneity itself. The displayed features
were selected from the extracted feature set. Full-series features include
test observations, making this a description of the obtained errors rather
than an evaluated pre-forecast model-selection rule.

\textbf{Evidence scope.} The analysis uses one selected computation per
configuration. Counts and comparisons were checked against selected task
records and metric summaries; raw forecast and metric arrays were not
independently inspected locally. Cross-configuration IQRs and correlations
describe this benchmark, not variability across repeated computations.
\end{document}
